...
const makeItSnow = ({
    ...
    for (let i = 0; i < Math.floor(element.clientWidth / 10); ++i) {
        const left = randomIntWithUnit(0, element.clientWidth, "px"),
            ...
            fallDuration = randomIntWithUnit(
                fallDurationRange[0],
                fallDurationRange[1],
                "ms"
            ),
            fallDelay = randomIntWithUnit(
                fallDelayRange[0],
                fallDelayRange[1],
                "ms"
            ),
            offset = randomIntWithUnit(offsetRange[0], offsetRange[1], "px"),
            offsetDuration = randomIntWithUnit(
                offsetDurationRange[0],
                offsetDurationRange[1],
                "ms"
            ),
            rotateDuration = randomIntWithUnit(
                rotateDurationRange[0],
                rotateDurationRange[1],
                "ms"
            );
