...
class Keyboard {
    constructor() {
        ...
        this.capsIndicatorElement = document.getElementById(
    ...
    capsOn = () => {
        this.capsIndicatorElement.classList.add(
            "keyboard-key-caps-indicator-active"
        );
    };

    capsOff = () => {
        this.capsIndicatorElement.classList.remove(
            "keyboard-key-caps-indicator-active"
        );
    };
    ...
    highlightKeysToPressFor = key => {
        ...
        for (const highlightedKeyElement of highlightedKeyElements) {
            ...
            highlightedKeyElement.classList.remove(
                "keyboard-key-text-highlight"
            );
        }

        const lessonCurrentElement =
            document.getElementsByClassName("lesson-current")[0];
        ...
        for (const keyElement of this.keyElements) {
            if (
                ...
                if (keyElement.dataset.value === key) {
                    ...
                    try {
                        keyElement
                            .getElementsByClassName("keyboard-key-alt")[0]
                    ...
                    if (
                        keyElement.classList.contains("keyboard-key-lpinkie") ||
                        ...
                        this.shifts[1].classList.add(
                            "keyboard-key-highlight",
                            "keyboard-key-text-highlight"
                        );
                    } else {
                        this.shifts[0].classList.add(
                            "keyboard-key-highlight",
                            "keyboard-key-text-highlight"
                        );
                    }
